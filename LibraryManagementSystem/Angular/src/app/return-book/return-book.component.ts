...
export class ReturnBookComponent implements OnInit {
  ...
  getBookInfo(bookInfo: NgForm) {
    this.http.get<BookTransactionResponse>(`http://localhost:8081/libms/library/librarian/getIssuedInfo/${bookInfo.value.id}`)
      .subscribe(response => {
        if (response != null) {
  ...
  returnBook(id) {
    this.http.delete(`http://localhost:8081/libms/library/librarian/returnBook/${id}`)
      .subscribe(response => {
        if (response) {
