...
export class RequestStatusComponent implements OnInit {
  ...
  getRequestStatusInfo(requestStatusInfo: NgForm) {
    this.http.get<BookTransactionResponse>(`http://localhost:8081/libms/requsetBook/${requestStatusInfo.value.id}`)
      .subscribe(response => {
        if (response != null) {
          alert('Request Accepted Succefully');
        } else {
          alert('Request rejected for ID ' + requestStatusInfo.value.id);
        }
